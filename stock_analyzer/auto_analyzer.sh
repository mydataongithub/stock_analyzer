...
# Automatische Ausführung des Stock Analyzers
...
# 0 5 * * 0 /home/ganzfrisch/finance/stock_analyzer/auto_analyzer.sh

# Pfade
ANALYZER_SCRIPT="/home/ganzfrisch/finance/stock_analyzer/run_analysis.sh"
LOG_DIR="/home/ganzfrisch/finance/stock_analyzer/logs"
LOG_FILE="$LOG_DIR/auto_analyzer_$(date +"%Y%m%d").log"

# E-Mail-Konfiguration (optional)
EMAIL_RECIPIENT="your.email@example.com"
EMAIL_SUBJECT="Stock Analyzer - Wöchentliche Analyse $(date +"%Y-%m-%d")"
EMAIL_BODY="Anbei die wöchentliche Aktienanalyse."
...
echo "===== AUTO STOCK ANALYZER =====" > "$LOG_FILE"
echo "Startzeit: $(date)" >> "$LOG_FILE"
echo "============================" >> "$LOG_FILE"
echo "" >> "$LOG_FILE"

# Ausführen des Haupt-Skripts
...
if [ $RESULT -eq 0 ]; then
    ...
    STATUS_MSG="Aktienanalyse erfolgreich abgeschlossen"
else
    echo "Status: FEHLER (Code: $RESULT)" >> "$LOG_FILE"
    STATUS_MSG="FEHLER bei der Aktienanalyse (Code: $RESULT)"
fi
...
echo "Analyse abgeschlossen. Log-Datei: $LOG_FILE"
